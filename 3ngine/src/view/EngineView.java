...
public class EngineView extends JComponent {
    ...
    public void paint(Graphics g) {
        ...
        if (this.engineController.isShowControlsOn()) this.drawControlPanel();
        
        // Show the debugging and framing info
        if (this.engineController.isShowTimesOn()) this.drawFramingInfoPanel();
    ...
    public void drawControlPanel() {
        ...
        g.fillRect(4, EngineModel.dimY-240, 384, 140);
        // text
        g.setColor(new Color(.5f,.8f,1f));
        g.setFont(new Font("Sans-serif", Font.PLAIN, 22));
        g.drawString("CONTROLS:", 10, EngineModel.dimY-214);
        g.setColor(Color.WHITE);
        g.setFont(new Font("Sans-serif", Font.PLAIN, 14));
        g.drawString("- WASD to move the camera around (at the same height)", 16, EngineModel.dimY-190);
        g.drawString("- Space/Ctrl to move the camera updwards or downwards", 16, EngineModel.dimY-174);
        g.drawString("- Directional arrows to pan and tilt the camera", 16, EngineModel.dimY-158);
        g.drawString("- \'O\' to open file explorer", 16, EngineModel.dimY-142);
        g.drawString("- 1,2 or 3 to select different rendering modes", 16, EngineModel.dimY-126);
        g.setColor(new Color(1f,0.5f,0.5f));
        g.drawString("- \'Esc\' to exit the program", 16, EngineModel.dimY-110);
    }
